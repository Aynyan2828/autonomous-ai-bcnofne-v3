...
def get_ai_mode_status() -> Dict[str, Any]:
    ...
    try:
        ...
        provider_mode = os.getenv("AI_PROVIDER_MODE", "local_preferred")
        ...
        # モデル名
        local_model = os.getenv("MODEL_CHAT", "qwen2.5:7b")
        ...
        last_switch_reason = _get_state(db, "last_ai_mode_switch_reason", "No failures recorded")
        
        # 日本語ラベル生成
        if provider_mode == "openai_only":
            display_label_ja = f"クラウドAI固定 ({openai_model})"
            display_label_en = f"Cloud AI only ({openai_model})"
            ...
            status_suffix = " (安定稼働)" if provider_mode == "local_only" else ""
            display_label_ja = f"ローカルAI ({local_model}){status_suffix}"
            display_label_en = f"Local AI ({local_model}){status_suffix}"
        else:
